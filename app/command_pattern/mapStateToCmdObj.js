...
export function storeCmd (func, command, parent = null) {
  ...
  else if (command.commandId === 4) commandInstance = new Speak(store.getState().challenges.sprite.name); //Speak
  ...
  else if (command.commandId === 9) commandInstance = new Loop(command.input); //Loop
